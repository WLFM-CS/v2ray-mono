...
def inbound2vmess(inbound):
    ...
    _add = _add or _host or _host_ip

    if "settings" in inbound and "clients" in inbound["settings"]:
        for c in inbound["settings"]["clients"]:
            vobj = dict(
                id=c["id"], aid=str(c["alterId"]),
                v="2", tls=_tls, add=_add, port=_port, type=_type, net=_net, path=_path, host=_host, ps="{}-{}".format(_path.strip("/").capitalize(), _host_ip).strip("/").strip("-"))
